...
export const configSchema = z.object({
    hfToken: z.string().optional().describe('Hugging Face API 토큰 (이미지 생성에 필요)')
})

// Smithery 배포용 createServer 함수
    ...
    server.tool(
        ...
        async ({ prompt }) => {
            try {
                // Hugging Face 토큰 확인
                if (!hfToken) {
                    throw new Error('Hugging Face 토큰이 설정되지 않았습니다. config에서 hfToken을 설정해주세요.')
                }
